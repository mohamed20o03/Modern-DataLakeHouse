...
import com.abdelwahab.ingestion_worker.consumer.rabbitmq.RabbitMQConsumer;
...
public class MessageConsumerFactory {
     ...
     * (default: {@code "ingestion.queue"}).
     *
     * @param host             broker hostname
     * @param port             broker port
     * @param username         broker username
     * @param password         broker password
     * @param ingestionService service that processes each received message
    ...
    public static MessageConsumer create(String host, int port, String username,
            String password, IngestionService ingestionService, int concurrency) throws Exception {
        ...
        String queueName = System.getenv().getOrDefault("RABBITMQ_QUEUE", "ingestion.queue");
        ...
        return switch (type) {
            ...
            case "rabbitmq" -> new RabbitMQConsumer(host, port, username, password,
                    ingestionService, concurrency, queueName);
